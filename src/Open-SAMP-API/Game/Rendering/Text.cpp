...
Game::Rendering::Text::Text(Renderer *renderer, const std::string& font, float iFontSize, bool Bold, bool Italic, float x,float y,D3DCOLOR color,const std::string& text, bool bShadow, bool bShow) : RenderBase(renderer), m_D3DFont(NULL)
{
	setPos2D(x, y);
	setColor(color);
	setText(text);
...
bool Game::Rendering::Text::updateText(const std::string& Font,float FontSize,bool Bold,bool Italic)
{
	m_Font = Font;
	m_FontSize = FontSize;
	m_bBold = Bold;
...
{
	float size = calculatedYPos(m_FontSize);

	m_D3DFont = std::make_shared<CD3DFont>(m_Font.c_str(), (DWORD)size, (m_bBold) ? D3DFONT_BOLD : 0 | (m_bItalic) ? D3DFONT_ITALIC : 0 | D3DFONT_FILTERED);
...
bool Game::Rendering::Text::drawText(float x, float y, DWORD dwColor, const std::string& strText, DWORD dwFlags /*= 0L*/)
{
